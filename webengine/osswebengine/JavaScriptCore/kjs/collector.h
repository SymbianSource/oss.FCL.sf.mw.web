...
namespace KJS {
  ...
  public:
    ...
    static size_t size();
#if PLATFORM(SYMBIAN)
    static bool isOutOfMemory() { return memoryFull || MemoryManager::Status() != ENoOOM; }
#else
    static bool isOutOfMemory() { return memoryFull; }
#endif

#if PLATFORM(SYMBIAN)
    static unsigned int CallStackGrowthThresh;
    static Vector<JSCell *> recursivelyOrphanedMarkTops;
#endif // PLATFORM(SYMBIAN)
    
    IMPORT static void protect(JSValue*);
    IMPORT static void unprotect(JSValue*);
    
    IMPORT static void collectOnMainThreadOnly(JSValue*);
  ...
  private:
    IMPORT static const CollectorBlock* cellBlock(const JSCell*);
    IMPORT static CollectorBlock* cellBlock(JSCell*);
    IMPORT static size_t cellOffset(const JSCell*);
    ...
    static void markRecursivelyOrphanedCells();
...
#if PLATFORM(SYMBIAN)
  // fixme need to figure out a way to allocate 64KB-aligned blocks in symbian,
